...
public class Dao {
	...
	public void adiciona(Evento evento) throws SQLException {
		...
		try {
			...
			//adicionaLigacoes(lastId, "usuarios_eventos");
	...
	public void adicionaLigacoes(int id, String table) throws SQLException {
		String sql = "SELECT id, tags FROM aluno;";
		try {
			PreparedStatement stmt = connection.prepareStatement(sql);
			...
			while (rs.next()) {
				//POSSO CALCULAR JACCARD AQUI PRA QUANDO UM EVENTO NOVO ENTRA
				int idUsu = rs.getInt("id");
				sql = "INSERT INTO "+table+" (id_usu, id_eve, peso, jaccard) VALUES ("+idUsu+","+id+","+"0,0);";
			}
		} catch (SQLException e) {
			throw new RuntimeException(e);
		}
	...
	public void adiciona(Materia materia) throws SQLException {
		...
		try {
			...
			//adicionaLigacoes(lastId, "usuarios_materias");
	...
	public void adiciona(Estagio estagio) throws SQLException {
		...
		try {
			...
			//adicionaLigacoes(lastId, "usuarios_estagio");
		} catch (SQLException e) {
			throw new RuntimeException(e);
		} finally {
			connection.close();
